...
async function main() {
  ...
  scheduler.register(
    createRetentionJob({
      ...
      recontactStateStore,
      lastInteractionStore,
      reportStore: new FileRetentionReportStore(config.retentionReportPath, config.retention.diasDeReportes),
  ...
  void (async () => {
    const corridas = await new FileRetentionReportStore(config.retentionReportPath).readAll().catch(() => []);
